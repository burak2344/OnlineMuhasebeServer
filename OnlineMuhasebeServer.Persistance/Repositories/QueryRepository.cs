...
{
	public class QueryRepository<T> : IQueryGenericRepository<T>
		where T : Entity
	{
		...
		private static readonly Func<Context.CompanyDbContext, Expression<Func<T, bool>>,bool , Task<T>>
			 GetFirstByExpiressionCompiled =
			EF.CompileAsyncQuery((Context.CompanyDbContext context, Expression<Func<T, bool>> expression, bool isTracking) => isTracking == true 
